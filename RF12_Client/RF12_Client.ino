...
static void homePage(BufferFiller& buf) {
    ...
    for (byte i = 0; i < NUM_MESSAGES; ++i) {
        ...
        if (history_len[j] > 0) {
            ...
            buf.emit_p(PSTR("\nMessage #$D$D$D$D received: "), // hack, to show leading zero's
                                n/1000, (n/100) % 10, (n/10) % 10, n % 10);
            for (byte k = 0; k < history_len[j]; ++k)
                buf.emit_raw((char*)(&history_rcvd[j][k]), 1);
        }
    }
    long t = millis() / 1000;
    word h = t / 3600;
...
void loop(){
    ...
    if (needToSend && rf12_canSend()) 
    {
        ...
        sprintf(payload, "WD05000\0");
        rf12_sendStart(0, payload, sizeof payload);
        rf12_sendWait(0);
        while (!rf12_canSend()) rf12_recvDone();
        sprintf(payload, "WD06190\0");
        rf12_sendStart(0, payload, sizeof payload);
        rf12_sendWait(0);
        while (!rf12_canSend()) rf12_recvDone();
